...
1. Осуществим вход в систему, используя свое имя пользователя.

2. Запишем в файл file.txt названия файлов, содержащихся в каталоге /etc. Допишим в этот же файл названия файлов, содержащихся в вашем домашнем каталоге и проверим его содержимое с помощью команды cat (рис.1).
...
![Вывод имен файлов](image/2.png){ #fig:001 width=70% }

4. Определим, какие файлы в вашем домашнем каталоге имеют имена, начинавшиеся с символа "c", используя несколько способов (рис.3-4).

![Поиск файлов](image/3.png){ #fig:001 width=70% }

![Поиск файлов](image/4.png){ #fig:001 width=70% }

5. Выведем на экран (по странично) имена файлов из каталога /etc, начинающиеся
с символа "h" (рис.5) с помощью команды find /etc -maxdepth | -name "h*" | less.

![Вывод имен файлов](image/5.png){ #fig:001 width=70% }

6. Запустим в фоновом режиме процесс, который будет записывать в файл ~/logfile
файлы, имена которых начинаются с log (рис.6-7).

![Команда](image/7.png){ #fig:001 width=70% }

![Содержимое файла logofile](image/6.png){ #fig:001 width=70% }

7. Удалим файл logofile командой rm logofile.

8. Запустим из консоли в фоновом режиме редактор gedit (рис.8).

![Запуск gedit](image/8.png){ #fig:001 width=70% }

9.Определим идентификатор процесса gedit, используя команду ps, конвейер и фильтр
grep (рис.9), затем завершим процесс gedit с помощью команды kill (рис.10)

![Статус gedit](image/10.png){ #fig:001 width=70% }

![Завершение процесса gedit](image/9.png){ #fig:001 width=70% }

10. Выполним команды df и du (рис.13-14), предварительно получив более подробную информацию
об этих командах, с помощью команды man (рис.11-12).

![Справка о команде df](image/12.png){ #fig:001 width=70% }

![Справка о команде du](image/13.png){ #fig:001 width=70% }

![Выполнение команды du](image/du.png){ #fig:001 width=70% }

![Выполнение команды df](image/14.png){ #fig:001 width=70% }

11. Выведем имена всех директорий, имеющихся в домашнем каталоге с помощью команды find ~ type d (рис.15)

![Вывод всех директорий](image/16.png){ #fig:001 width=70% }
